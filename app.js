...
var dayRightnow= new Date().getDate();
    ...
    for (let i = yearRightnow; i >= yearRightnow-90; i--) {
...
for ( var i=0 ; i < monthRightnow; i++){
    ...
    var fullOldDays = oldDays + monthDays2 - monthDays1 + dayRightnow - daySelected;
        if (fullOldDays >= 6571) {
            swal ( "You are over 18" ,  "Welcome to play with us" ,  "success");
        
            window.location = "https://www.national-lottery.co.uk/";
        } else {
            swal ( "Oops,You are so young to start playing." ,  "Welcome back when you are over 18." ,  "error")        
        }
    });
});